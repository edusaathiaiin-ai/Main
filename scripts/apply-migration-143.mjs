// One-off applier for supabase/migrations/143_align_education_institution_stats_cache.sql
// Same pattern as apply-migration-142.mjs.
//
// Usage:
//   DATABASE_URL=$(doppler secrets get DATABASE_URL --plain --project edusaathiai --config prd) \
//     node scripts/apply-migration-143.mjs
...
const sqlPath = resolve(here, '..', 'supabase', 'migrations', '143_align_education_institution_stats_cache.sql');
...
try {
  ...
  `);
  console.log('Applied. Final columns:');
  console.table(rows);
} catch (err) {
