...
export const GENERIC_DOMAINS = new Set([
  ...
  'live.com', 'rediffmail.com', 'protonmail.com', 'aol.com', 'icloud.com',
  'mail.com', 'ymail.com', 'msn.com', 'zoho.com',
]);
...
export async function validateFacultyEmail(
  ...
  if (employment_status === 'retired') {
    return {
      allowed: true,
      status: 'skipped',
      message: 'Any email accepted for retired faculty. Verification via document upload.',
    };
  }
  ...
  if (domain.endsWith('.ac.in') || domain.endsWith('.edu') || domain.endsWith('.edu.in')) {
    return {
      allowed: true,
      status: 'pending_review',
      message: 'Academic email detected. Our team will verify your institution within 48 hours.',
    };
  }
  ...
  return {
    allowed: true,
    status: 'pending_review',
    message: 'Your email domain is not in our verified list. Our team will review your application within 48 hours.',
  };
}
