...
export function DailyChallengeWidget({ saathiId, primaryColor = '#C9993A' }: Props) {
  const [open, setOpen] = useState(false);
  const [challenge, setChallenge] = useState<Challenge | null>(null);
  const [attempt, setAttempt] = useState<Attempt | null>(null);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<number | null>(null);
  const [result, setResult] = useState<{ is_correct: boolean; correct_option: number; explanation: string; streak: number } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) return;
      const payload = await res.json() as { challenge: Challenge; attempt: Attempt | null };
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/daily-challenge`,
        {
          ...
          body: JSON.stringify({ challenge_id: challenge.id, selected_option: optionIndex }),
        }
      );
      if (!res.ok) return;
      const data = await res.json() as { is_correct: boolean; correct_option: number; explanation: string; streak: number };
      setResult(data);
      setAttempt({ selected_option: optionIndex, is_correct: data.is_correct, streak_count: data.streak });
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <>
      ...
      >
        ...
        <div style={{ textAlign: 'left' }}>
          <p style={{ fontSize: '11px', fontWeight: 700, color: alreadyDone ? '#22C55E' : primaryColor, margin: 0, lineHeight: 1.2 }}>
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    <p style={{ fontSize: '13px', fontWeight: 700, color: primaryColor, margin: 0 }}>
                      Daily Challenge
                    </p>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '18px', flexWrap: 'wrap' }}>
                {challenge.topic && (
                  <span style={{
                    ...
                    background: `${primaryColor}18`, border: `0.5px solid ${primaryColor}40`,
                    color: primaryColor,
                  }}>
                    {challenge.topic}
                  </span>
              ...
              {/* Question */}
              <p style={{
                fontSize: '15px', color: '#fff', lineHeight: 1.65,
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
                {(challenge.options as string[]).map((opt, i) => {
                  ...
                  if (result) {
                    if (isCorrect) { bg = 'rgba(34,197,94,0.12)'; border = '#22C55E55'; color = '#4ADE80'; }
                    else if (isWrong) { bg = 'rgba(239,68,68,0.12)'; border = '#EF444455'; color = '#FCA5A5'; }
                    else { bg = 'rgba(255,255,255,0.02)'; color = 'rgba(255,255,255,0.3)'; }
                  } else if (isSelected) {
                    bg = `${primaryColor}18`; border = `${primaryColor}60`; color = primaryColor;
                  }
                  ...
                  return (
                    ...
                    >
                      <span style={{
                        ...
                        background: result && isCorrect ? '#22C55E' : result && isWrong ? '#EF4444' : `${primaryColor}30`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: '11px', fontWeight: 700, color: result ? '#fff' : primaryColor,
                        flexShrink: 0,
                      }}>
                        {result && isCorrect ? '✓' : result && isWrong ? '✗' : String.fromCharCode(65 + i)}
                      </span>
                      <span style={{ fontSize: '14px', color, lineHeight: 1.5 }}>{opt}</span>
                    </motion.button>
                  );
                })}
              </div>
              ...
              <AnimatePresence>
                {result && (
                  <motion.div
                    ...
                    style={{
                      background: result.is_correct ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)',
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                      <span style={{ fontSize: '18px' }}>{result.is_correct ? '🎉' : '💡'}</span>
                      <p style={{ fontSize: '13px', fontWeight: 700, color: result.is_correct ? '#4ADE80' : '#FCA5A5', margin: 0 }}>
                        {result.is_correct ? 'Correct!' : 'Not quite — but you learned!'}
                      </p>
                      {result.streak > 0 && (
                        <span style={{
                          marginLeft: 'auto', fontSize: '11px', fontWeight: 700,
                          padding: '2px 8px', borderRadius: '20px',
                          background: '#F59E0B20', border: '0.5px solid #F59E0B40',
                          color: '#F59E0B',
                        }}>
