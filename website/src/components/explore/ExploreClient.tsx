...
export function ExploreClient({ saathiId }: Props) {
  ...
  const fetchResources = useCallback(async (forceRefresh: boolean) => {
  ...
  useEffect(() => { fetchResources(false); }, [fetchResources]);
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            ...
            {updatedLabel && (
              <p style={{
                ...
                {cached ? 'Cached · ' : ''}{`Week ${week} · ${updatedLabel}`}
              </p>
            )}
          </div>
        </div>
        ...
        <>
          ...
          {featured.length > 0 && (
            <section style={{ marginBottom: '40px' }}>
              <SectionHeading>✦ Your Saathi&apos;s top picks this week</SectionHeading>
              <div style={{
                display: 'grid',
                ...
                {featured.map((r, i) => (
                  <FeaturedCard key={r.id} resource={r} color={color} index={i} />
                ))}
              </div>
            </section>
          )}
          ...
          {filtered.length === 0 && (
            <p style={{ textAlign: 'center', padding: '48px', color: 'rgba(255,255,255,0.3)' }}>
          ...
          <div style={{
            ...
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', margin: '0 0 6px' }}>
              ✦ These resources exist outside the EdUsaathiAI universe
            </p>
            <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.18)', margin: 0, lineHeight: 1.6 }}>
              Your Saathi curates them weekly using the latest knowledge.
              Click any card to open in a new tab. Resources refresh every Monday
              or when you tap Refresh.
            </p>
          </div>
        </>
      )}
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: `${color}12`, filter: 'blur(18px)',
        pointerEvents: 'none',
      }} />

      {/* Top row: emoji + badges */}
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '10px' }}>
        <span style={{ fontSize: '26px', lineHeight: 1, flexShrink: 0 }}>{resource.emoji}</span>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', marginBottom: '5px' }}>
            <Badge bg={`${color}20`} fg={color}>✦ Top Pick</Badge>
            {typeConf && (
            ...
            {resource.is_free && (
              <Badge bg="rgba(74,222,128,0.1)" fg="#4ADE80">Free</Badge>
            )}
            {resource.is_indian_context && (
              <Badge bg="rgba(251,146,60,0.1)" fg="#FB923C">🇮🇳 India</Badge>
            )}
          </div>
          <h3 style={{
      ...
      {(resource.author ?? resource.year) && (
        <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.28)', margin: '0 0 10px' }}>
      ...
      <span style={{
        ...
        Explore →
        <span style={{ fontSize: '9px', color: 'rgba(255,255,255,0.2)', fontWeight: '400' }}>
          new tab
        </span>
      </span>
    </motion.a>
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        justifyContent: 'space-between', marginBottom: '9px',
      }}>
        <span style={{
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          ...
          {resource.is_indian_context && (
            <Badge bg="rgba(251,146,60,0.1)" fg="#FB923C">🇮🇳</Badge>
          )}
        </div>
      </div>
      ...
      {/* Author / year */}
      {(resource.author ?? resource.year) && (
        <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.22)', margin: '0 0 8px' }}>
          {[resource.author, resource.year].filter(Boolean).join(' · ')}
        </p>
      )}
...
if (typeof document !== 'undefined') {
  ...
  if (!document.getElementById(styleId)) {
    ...
    el.id = styleId;
    el.textContent = '@keyframes explore-spin { to { transform: rotate(360deg); } }';
