...
function sm2(
  card: Flashcard,
  quality: 0 | 3 | 5
): Pick<Flashcard, 'ease_factor' | 'interval_days' | 'repetitions' | 'next_review_date'> {
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
            <span style={{
              fontSize: '9px', fontWeight: 700, color: primaryColor,
          ...
          <p style={{ fontSize: '15px', color: '#fff', lineHeight: 1.6, flex: 1, margin: 0 }}>
            {card.front || '(empty front)'}
          </p>
          <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.2)', marginTop: '12px', margin: '12px 0 0' }}>
            Tap to reveal →
          </p>
        </div>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '10px', fontWeight: 700, color: primaryColor, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '12px' }}>
            Answer
          </p>
          <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)', lineHeight: 1.65, flex: 1, margin: '0 0 16px', whiteSpace: 'pre-wrap' }}>
            {card.back}
          </p>
...
export function FlashcardsClient({ saathiId }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060F1D', color: '#fff', paddingBottom: '80px' }}>
      {/* Header */}
      <div style={{
        padding: '20px 20px 0',
        background: '#060F1D',
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
            <Link href="/chat" style={{
              color: 'rgba(255,255,255,0.35)', textDecoration: 'none', fontSize: '13px',
              display: 'flex', alignItems: 'center', gap: '4px',
            }}>
              ← Chat
            </Link>
            <div style={{ flex: 1 }}>
              <h1 style={{ fontSize: '22px', fontFamily: 'var(--font-playfair)', color: '#fff', margin: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '20px', fontWeight: 700, color: primaryColor, margin: 0 }}>{cards.length}</p>
              <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)', margin: 0 }}>Total</p>
            </div>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#F59E0B', margin: 0 }}>{dueCards.length}</p>
              <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)', margin: 0 }}>Due today</p>
            </div>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#4ADE80', margin: 0 }}>
                {cards.filter((c) => c.repetitions >= 3).length}
              </p>
              <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)', margin: 0 }}>Mastered</p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0', borderBottom: 'none' }}>
            {(['due', 'all'] as const).map((t) => (
              ...
              >
                {t === 'due' ? `Due (${dueCards.length})` : `All (${cards.length})`}
              </button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          <div style={{ display: 'flex', justifyContent: 'center', paddingTop: '60px' }}>
            <div style={{
              ...
              border: `2px solid ${primaryColor}30`, borderTop: `2px solid ${primaryColor}`,
              animation: 'spin 0.8s linear infinite',
            }} />
          </div>
        ) : displayCards.length === 0 ? (
          <div style={{ textAlign: 'center', paddingTop: '60px' }}>
            <p style={{ fontSize: '40px', marginBottom: '12px' }}>{tab === 'due' ? '🎉' : '🃏'}</p>
            ...
            {tab === 'due' && cards.length > 0 && (
              <button
                ...
                style={{
                  ...
                  background: `${primaryColor}20`, border: `0.5px solid ${primaryColor}40`,
                  color: primaryColor, fontSize: '13px', fontWeight: 600, cursor: 'pointer',
                }}
              >
                Browse all cards
              </button>
