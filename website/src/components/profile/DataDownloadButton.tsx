...
export default function DataDownloadButton({ userId }: DataDownloadButtonProps) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      const [
        ...
        supabase.from('chat_messages').select('id,role,content,created_at').eq('user_id', userId).order('created_at', { ascending: false }).limit(100),
  ...
  return (
    <button
      ...
      disabled={loading}
      className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all hover:brightness-110 disabled:opacity-60"
