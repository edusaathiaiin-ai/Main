        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          {statement.length > 0 && statement.length < 50 && (
            <p className="text-[10px]" style={{ color: '#F59E0B' }}>
              Write at least {50 - statement.length} more chars
            </p>
          )}
        </div>
    ...
    >
            ...
            {saathi && (
              <span
                ...
                style={{
                  ...
                  color: saathi.accent,
                  border: `0.5px solid ${saathi.primary}40`,
                }}
              >
      ...
      <div className="mb-4 flex flex-wrap gap-1.5">
        {project.includes_authorship && (
          <BenefitChip icon="✍️" label="Co-authorship" highlight />
        )}
        {project.includes_stipend && project.stipend_amount && (
          <BenefitChip
            icon="💰"
            label={`₹${project.stipend_amount.toLocaleString('en-IN')}/mo`}
            highlight
          />
        )}
        {project.includes_certificate && (
          <BenefitChip icon="📜" label="Certificate" />
        )}
        {project.includes_letter && (
          <BenefitChip icon="📄" label="Recommendation letter" />
        )}
        {project.duration_months && (
          <BenefitChip
            icon="🗓"
            label={`${project.duration_months}m duration`}
          />
        )}
      </div>
...
export default function ResearchPage() {
  ...
  async function loadProjects() {
    ...
    const enriched: ResearchProject[] = (projectData as ResearchProject[]).map(
      (p) => ({
        ...p,
    ...
    setProjects((prev) =>
      prev.map((p) =>
        p.id === successId || p.id === applyTarget?.id
          ? {
              ...p,
              my_application: { status: RESEARCH_APPLICATION_STATUS.PENDING },
              total_applicants: p.total_applicants + 1,
            }
          : p
      )
    )
  ...
  const filtered = projects.filter((p) => {
    if (selectedVertical !== 'all' && p.vertical_id !== selectedVertical)
      return false
