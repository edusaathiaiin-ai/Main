...
function CompanyApplyModal({
  ...
  async function submit() {
    ...
    const { data: soul } = await supabase
      .from('student_soul')
      .select(
        'depth_calibration, future_research_area, top_topics, enrolled_subjects'
      )
      ...
      >
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            value={coverNote}
            onChange={(e) => {
              if (e.target.value.length <= MAX) setCoverNote(e.target.value)
            }}
...
function ResearchApplyModal({
  ...
  async function submit() {
    if (!profile || statement.trim().length < 30) {
      setError(
        'Please write at least 30 characters for your research statement.'
      )
      return
    }
    setSubmitting(true)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              value={whyProject}
              onChange={(e) => {
                if (e.target.value.length <= 200) setWhyProject(e.target.value)
              }}
        ...
        >
          After submitting, {posterName} will see your soul profile including
          depth, research dream, and top topics. You&apos;ll be notified when
          they respond.
        </p>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={submit}
            disabled={submitting || statement.trim().length < 30}
            className="flex-1 rounded-xl py-2.5 text-sm font-bold"
            style={{
              background:
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            ...
            {posting.faculty_profile?.designation
              ? ` · ${posting.faculty_profile.designation}`
              : ''}
            {posting.faculty_profile?.institution_name
              ? ` · ${posting.faculty_profile.institution_name}`
              : ''}
          </p>
        </div>
      ...
      {posting.research_area && (
        <p className="mb-2 text-xs font-medium" style={{ color: '#C084FC' }}>
          🔬 {posting.research_area}
        </p>
      )}
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {seatsLeft > 0
              ? `${seatsLeft} seat${seatsLeft > 1 ? 's' : ''} left`
              : 'Full'}{' '}
            · {posting.total_applications} applied
          </span>
        </div>
        {!applied ? (
          <button
...
export default function InternshipsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [{ data: soul }, { data: postingData }, { data: myApps }] =
      await Promise.all([
        supabase
          .from('student_soul')
          .select(
    ...
    const enriched: Posting[] = (postingData ?? []).map((p) => {
      ...
      return {
        ...p,
        faculty_profile: Array.isArray(p.faculty_profile)
          ? (p.faculty_profile[0] ?? null)
          : p.faculty_profile,
      ...
      prev.map((p) =>
        p.id === postingId
          ? {
              ...
              total_applications: p.total_applications + 1,
            }
          : p
      )
    )
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <AnimatePresence>
          {successPostingId && (
            ...
            >
              ✓ Application submitted! You&apos;ll be notified when they
              respond.
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {[
            {
              ...
              count: companyPostings.length,
            },
            {
              id: 'research' as const,
              label: '🔬 Research Positions',
              count: researchPostings.length,
            },
          ].map((tab) => (
            <button
              key={tab.id}
          ...
          {verticals.map(
            (s) =>
              s && (
                <button
                  key={s.id}
                  onClick={() => setFilterVertical(s.id)}
                  className="rounded-full px-3 py-1.5 text-xs font-semibold"
                  style={{
                    background:
                      filterVertical === s.id
                ...
                return (
                  ...
                  >
                    ...
                    {app.status === 'selected' && (
                      ...
                      >
                        🎉 Congratulations! You were selected. This will be
                        added to your soul profile.
                      </div>
                    )}
                  </div>
                )
