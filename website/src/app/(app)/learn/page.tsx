...
function IntentCard({
  ...
  const daysLeft = Math.max(0, Math.round(
    (new Date(intent.expires_at).getTime() - Date.now()) / 86400000
  ));
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
        <div style={{ flex: 1, marginRight: '12px' }}>
          {saathi && (
            <span style={{
              fontSize: '10px', fontWeight: '600', color: saathi.accent,
              background: `${saathi.accent}18`, border: `0.5px solid ${saathi.accent}40`,
              borderRadius: '100px', padding: '2px 8px', display: 'inline-block', marginBottom: '6px',
            }}>
              {saathi.emoji} {saathi.name}
            </span>
          )}
          <p style={{ fontSize: '16px', fontWeight: '700', color: '#fff', margin: 0, lineHeight: 1.3 }}>
            {intent.topic}
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
          <span style={{ fontSize: '14px' }}>🔥</span>
          <span style={{ fontSize: '18px', fontWeight: '800', color: '#FB923C' }}>{intent.joiner_count}</span>
          <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)' }}>want this</span>
        </div>
      </div>

      {/* Description */}
      {intent.description && (
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.55)', margin: '0 0 12px', lineHeight: 1.5 }}>
          {intent.description}
        </p>
      )}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '14px' }}>
        <span style={{
          fontSize: '10px', fontWeight: '600', color: DEPTH_COLOR[intent.depth_preference],
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        {isOwn ? (
          <span style={{
            fontSize: '12px', fontWeight: '600', color: '#C9993A',
          ...
          <button
            ...
            style={{
              ...
              background: 'rgba(251,146,60,0.1)', border: '0.5px solid rgba(251,146,60,0.35)',
...
function DeclareForm({ onSuccess, defaultVerticalId }: { onSuccess: () => void; defaultVerticalId: string }) {
  ...
  if (done) {
    const shareText = encodeURIComponent(`I want to learn "${topic}" on EdUsaathiAI. Join my intent and let's get a professor to teach this! 🎓`);
    ...
    return (
      ...
      >
        ...
        <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '22px', fontWeight: '700', color: '#fff', margin: '0 0 8px' }}>
          Your learning intent is live!
        </h3>
        <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.5)', margin: '0 0 6px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxWidth: '280px', margin: '0 auto' }}>
          <button
            onClick={onSuccess}
            style={{
              background: '#C9993A', color: '#060F1D', fontWeight: '700',
              fontSize: '14px', borderRadius: '10px', padding: '12px 24px',
          ...
          <a
            ...
            style={{
              ...
              fontSize: '13px', borderRadius: '10px', padding: '12px 24px',
  ...
  const chipBtn = (active: boolean, color: string = '#C9993A'): React.CSSProperties => ({
    padding: '8px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: active ? '700' : '400',
    cursor: 'pointer', border: `0.5px solid ${active ? color : 'rgba(255,255,255,0.12)'}`,
    background: active ? `${color}20` : 'rgba(255,255,255,0.04)',
    color: active ? color : 'rgba(255,255,255,0.55)', transition: 'all 0.15s',
  });
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 1 && (
          <motion.div key="s1" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }}>
            <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '20px', fontWeight: '700', color: '#fff', margin: '0 0 6px' }}>
            ...
            <select
              ...
              style={{ ...inputStyle, marginBottom: '16px', appearance: 'none' }}
            >
              {SAATHIS.map((s) => (
            ...
            />
            <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.25)', margin: '0 0 20px', textAlign: 'right' }}>
              {topic.length}/100
            </p>
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Tags</label>
                <button
                  onClick={fetchTagSuggestions}
                  disabled={loadingTags || topic.length < 5}
                  style={{
                    fontSize: '11px', color: '#C9993A', background: 'transparent',
                    border: 'none', cursor: 'pointer', opacity: topic.length < 5 ? 0.4 : 1,
                  }}
                >
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '8px' }}>
                  {suggestedTags.map((tag) => (
                    <button
                      ...
                      style={{
                        fontSize: '11px', borderRadius: '100px', padding: '3px 10px', cursor: 'pointer',
                        border: `0.5px solid ${tags.includes(tag) ? '#C9993A' : 'rgba(255,255,255,0.15)'}`,
                        background: tags.includes(tag) ? 'rgba(201,153,58,0.15)' : 'rgba(255,255,255,0.04)',
                        color: tags.includes(tag) ? '#C9993A' : 'rgba(255,255,255,0.5)',
                      }}
                    >
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '8px' }}>
                  {tags.map((tag) => (
                    <span
                      ...
                      onClick={() => toggleTag(tag)}
                      style={{
                        fontSize: '11px', color: '#C9993A', background: 'rgba(201,153,58,0.12)',
                        border: '0.5px solid rgba(201,153,58,0.35)', borderRadius: '100px',
            ...
            <button
              ...
              disabled={!topic.trim()}
              style={{
                width: '100%', background: '#C9993A', color: '#060F1D', fontWeight: '700',
                fontSize: '14px', borderRadius: '10px', padding: '13px', border: 'none',
                cursor: topic.trim() ? 'pointer' : 'not-allowed', opacity: topic.trim() ? 1 : 0.45,
              }}
            >
              Next →
            </button>
        ...
        {step === 2 && (
          <motion.div key="s2" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }}>
            ...
            <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: '8px' }}>Depth</label>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
              {(['beginner','intermediate','advanced'] as DepthPref[]).map((d) => (
                <button key={d} onClick={() => setDepth(d)} style={chipBtn(depth === d)}>{DEPTH_LABEL[d]}</button>
              ))}
            </div>

            <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: '8px' }}>Format</label>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '28px', flexWrap: 'wrap' }}>
              {(['any','lecture','series','onetoone','workshop'] as FormatPref[]).map((f) => (
                <button key={f} onClick={() => setFormat(f)} style={chipBtn(format === f)}>{FORMAT_LABEL[f]}</button>
              ))}
            </div>

            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setStep(1)} style={{ ...chipBtn(false), flex: 1 }}>← Back</button>
              <button onClick={() => setStep(3)} style={{
                flex: 2, background: '#C9993A', color: '#060F1D', fontWeight: '700',
                fontSize: '14px', borderRadius: '10px', padding: '13px', border: 'none', cursor: 'pointer',
              }}>Next →</button>
            </div>
          </motion.div>
        )}
        ...
        {step === 3 && (
          <motion.div key="s3" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', flexWrap: 'wrap' }}>
              {PRICE_OPTIONS.map((p) => (
                <button key={p.value} onClick={() => setMaxPrice(p.value)} style={chipBtn(maxPrice === p.value)}>
                  {p.label}
                </button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '28px', flexWrap: 'wrap' }}>
              {(['this_month','next_3_months','anytime'] as Urgency[]).map((u) => (
                <button key={u} onClick={() => setUrgency(u)} style={chipBtn(urgency === u)}>{URGENCY_LABEL[u]}</button>
              ))}
            </div>

            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setStep(2)} style={{ ...chipBtn(false), flex: 1 }}>← Back</button>
        ...
        {step === 4 && (
          <motion.div key="s4" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }}>
            ...
            {/* Preview card */}
            <div style={{
              background: 'rgba(201,153,58,0.06)', border: '0.5px solid rgba(201,153,58,0.25)',
              borderRadius: '14px', padding: '18px', marginBottom: '24px',
            }}>
              <p style={{ fontSize: '17px', fontWeight: '700', color: '#fff', margin: '0 0 6px' }}>{topic}</p>
              {description && (
                <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', margin: '0 0 10px', lineHeight: 1.5 }}>{description}</p>
              )}
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '8px' }}>
                {tags.map((tag) => (
                  <span key={tag} style={{
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                <span style={{ fontSize: '10px', color: DEPTH_COLOR[depth], background: `${DEPTH_COLOR[depth]}15`, border: `0.5px solid ${DEPTH_COLOR[depth]}40`, borderRadius: '100px', padding: '2px 8px' }}>
                  {DEPTH_LABEL[depth]}
                </span>
                <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.45)', background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '100px', padding: '2px 8px' }}>
                  {FORMAT_LABEL[format]}
                </span>
                <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.45)', background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '100px', padding: '2px 8px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setStep(3)} style={{ ...chipBtn(false), flex: 1 }}>← Back</button>
              <button
...
export default function LearnPage() {
  ...
  const loadIntents = useCallback(async () => {
    ...
    if (filterVertical !== 'all') query = query.eq('vertical_id', filterVertical);
    ...
    if (rows.length === 0) { setIntents([]); setLoading(false); return; }
    ...
    const enriched: LearningIntent[] = rows.map((r) => {
      const p = profileMap[r.student_id] as { full_name: string | null; institution_name: string | null; current_semester: string | null } | undefined;
      return {
        ...r,
  ...
  useEffect(() => {
    async function run() { await loadIntents(); }
    void run();
  }, [loadIntents]);
  ...
  async function handleJoin(intentId: string) {
    ...
    const supabase = createClient();
    await supabase.from('intent_joiners').insert({ intent_id: intentId, student_id: profile.id });
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060F1D', color: '#fff', fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '32px 16px 80px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
            ...
            <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '28px', fontWeight: '700', color: '#fff', margin: 0 }}>
              I Want to Learn
            </h1>
          </div>
          <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.45)', margin: '4px 0 0' }}>
        ...
        <div style={{
          ...
          {([['browse', '📢 Browse Intents'], ['declare', '+ Declare Intent']] as [string, string][]).map(([id, label]) => (
            <button
              key={id}
              onClick={() => setTab(id as 'browse' | 'declare')}
              style={{
                flex: 1, padding: '10px', borderRadius: '8px', border: 'none',
                cursor: 'pointer', fontSize: '13px', fontWeight: tab === id ? '600' : '400',
          ...
          <>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '20px' }}>
              ...
              >
                ...
                {SAATHIS.map((s) => (
                  <option key={s.id} value={s.id} style={{ background: '#0B1F3A' }}>{s.emoji} {s.name}</option>
                ))}
              </select>
              ...
              >
                <option value="all" style={{ background: '#0B1F3A' }}>Any format</option>
                {(['lecture','series','workshop','onetoone'] as FormatPref[]).map((f) => (
                  <option key={f} value={f} style={{ background: '#0B1F3A' }}>{FORMAT_LABEL[f]}</option>
                ))}
              </select>

              <select
                value={filterDepth}
                onChange={(e) => setFilterDepth(e.target.value as DepthPref | 'all')}
                style={{
                  background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.12)',
                  color: '#fff', borderRadius: '8px', padding: '6px 10px', fontSize: '12px', cursor: 'pointer',
                }}
              >
                <option value="all" style={{ background: '#0B1F3A' }}>Any depth</option>
                {(['beginner','intermediate','advanced'] as DepthPref[]).map((d) => (
                  <option key={d} value={d} style={{ background: '#0B1F3A' }}>{DEPTH_LABEL[d]}</option>
                ))}
              </select>
              ...
              >
                <option value="most_wanted" style={{ background: '#0B1F3A' }}>Most wanted</option>
                <option value="newest" style={{ background: '#0B1F3A' }}>Newest</option>
                <option value="expiring" style={{ background: '#0B1F3A' }}>Expiring soon</option>
              </select>
            </div>
            ...
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '60px 0' }}>
                ...
                <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.5)', marginBottom: '8px' }}>
                ...
                <button
                  ...
                  style={{
                    marginTop: '16px', background: '#C9993A', color: '#060F1D',
                ...
                {myIntents.length > 0 && (
                  <>
                    <div style={{ marginTop: '32px', marginBottom: '16px' }}>
                      <p style={{ fontSize: '11px', fontWeight: '600', color: 'rgba(255,255,255,0.3)', letterSpacing: '1px', textTransform: 'uppercase', margin: 0 }}>
                        Your intents
                      </p>
                    </div>
