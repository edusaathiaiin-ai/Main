  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          🔥 {intent.joiner_count}{' '}
          {intent.joiner_count === 1 ? 'student' : 'students'}
        </span>
      </div>
...
export default function FacultyDemandPage() {
  ...
  const [sortBy, setSortBy] = useState<
    'most_wanted' | 'highest_budget' | 'expiring'
  >('most_wanted')
  ...
  const fetchIntents = useCallback(async () => {
    async function run() {
      ...
      if (sortBy === 'most_wanted')
        ...
        query = query.order('max_price_paise', { ascending: false })
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        >
          {totalDemand} open learning intents · Students waiting for sessions
          like yours
        </p>
      </div>
      ...
      >
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px' }}>
          {(['most_wanted', 'highest_budget', 'expiring'] as const).map(
            (opt) => (
              <button
                key={opt}
      ...
      >
        ...
        >
          <strong style={{ color: '#C9993A' }}>How this works:</strong> When you
          create a session from an intent, all students who joined that intent
          are notified first and get priority booking for 24 hours. After that,
          the session opens to all students. You earn per seat booked.
        </p>
      </div>
    </div>
  )
