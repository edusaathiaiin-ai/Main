...
const LISTING_PLANS = [
  {
    ...
    label: 'Basic',
    price: '₹999',
    desc: 'Standard listing, manual browsing',
  },
  {
    id: 'featured',
    label: 'Featured ★',
    price: '₹2,999',
    desc: 'Top placement + top-10 matches delivered to your inbox',
    recommended: true,
  },
  {
    id: 'corporate',
    label: 'Corporate',
    price: '₹4,999/mo',
    desc: 'Unlimited postings + priority support',
  },
] as const
...
export default function InstitutionPage() {
  ...
  const [ifSelectedPosting, setIfSelectedPosting] =
    useState<InternPosting | null>(null)
  ...
  const [ifForm, setIfForm] = useState({
    ...
    preferred_subjects_raw: '',
    duration_months: 3,
    stipend_monthly: '',
    is_paid: false,
    offers_coauthorship: false,
    offers_certificate: true,
    location: '',
    is_remote: false,
    work_mode: 'onsite' as (typeof WORK_MODES)[number],
    total_seats: 1,
    ...
    async function load() {
      ...
      const [{ data: instData }, { data: listData }] = await Promise.all([
        ...
        supabase
          ...
          .select('*')
          .eq('institution_user_id', profile!.id)
  ...
  async function postListing() {
    ...
    const { data: listing } = await supabase
      ...
      .insert({
        ...
        skills_needed: form.skills_needed_raw
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean),
        stipend_amount: form.stipend_amount
          ? parseInt(form.stipend_amount)
          : null,
  ...
  async function loadIfApplicants(posting: InternPosting) {
    ...
    const { data } = await supabase
      ...
      .select(
        'id, posting_id, student_id, cover_note, research_statement, match_score, status, soul_snapshot, created_at'
      )
  ...
  async function postIfListing() {
    ...
    setIfPosting(true)
    const supabase = createClient()
    const { error: ifInsertError } = await supabase
      .from('internship_postings')
      .insert({
        ...
        preferred_subjects: ifForm.preferred_subjects_raw
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean),
        duration_months: ifForm.duration_months,
        stipend_monthly:
          ifForm.is_paid && ifForm.stipend_monthly
            ? parseInt(ifForm.stipend_monthly, 10)
            : null,
    ...
    setTimeout(() => {
      setIfPosted(false)
      setView('intern_finder')
    }, 2500)
  }

  const setIfF = <K extends keyof typeof ifForm>(
    key: K,
    val: (typeof ifForm)[K]
          ...
          <div>
            ...
            <AnimatePresence>
              {posted && (
                ...
                >
                  ✓ Posted! Matching engine running — top students will be
                  notified.
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="max-w-2xl space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setF('required_min_profile_pct', parseInt(e.target.value))
                  }
                  className="w-full"
                />
                <p
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <div>
                  ...
                  >
                    <span
                      className="relative flex h-5 w-10 items-center rounded-full transition-all"
                      style={{
              ...
              <button
                onClick={postListing}
                disabled={
                  !form.title.trim() || !form.description.trim() || posting
                }
                className="w-full rounded-xl py-4 text-base font-bold transition-all disabled:cursor-not-allowed disabled:opacity-40"
                style={{ background: '#C9993A', color: '#060F1D' }}
              >
                      ...
                      <div className="mb-2 flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          >
                            ...
                            {p.is_paid && p.stipend_monthly
                              ? ` · ₹${p.stipend_monthly.toLocaleString('en-IN')}/mo`
                              : ' · Unpaid'}
                            {p.duration_months
                              ? ` · ${p.duration_months}m`
                              : ''}
        ...
        {view === 'intern_finder_post' && (
          <div>
            ...
            {ifPosted && (
              ...
              >
                ✓ Posted! Soul-matching is running. Top students will be
                notified.
              </div>
            )}
            ...
            <div className="max-w-2xl space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  max={90}
                  step={5}
                  value={ifForm.min_depth}
                  onChange={(e) =>
                    setIfF('min_depth', parseInt(e.target.value))
                  }
                  className="w-full"
                />
                <p
                ...
                <div className="flex flex-wrap gap-2">
                  {['any', 'bachelor', 'masters', 'phd'].map((lvl) => (
                    <button
                      key={lvl}
                      type="button"
                      onClick={() => setIfF('min_academic_level', lvl)}
                      className="rounded-full px-4 py-2 text-sm font-medium transition-all"
                      style={{
                        background:
              ...
              <div>
                ...
                <input
                  value={ifForm.preferred_subjects_raw}
                  onChange={(e) =>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[1, 2, 3, 6].map((d) => (
                      <button
                        key={d}
                        type="button"
                        onClick={() => setIfF('duration_months', d)}
                        className="rounded-lg px-3 py-2 text-sm transition-all"
                        style={{
                          background:
              ...
              <div>
                ...
                <div className="mb-2 flex items-center gap-3">
                  <button
                    onClick={() => setIfF('is_paid', !ifForm.is_paid)}
              ...
              <div>
                ...
                <input
                  type="date"
                  value={ifForm.application_deadline}
                  onChange={(e) =>
                  ...
                  {LISTING_PLANS.map((plan) => (
                    <button
                      ...
                      onClick={() =>
                        setIfF(
                          'listing_plan',
                          plan.id as 'basic' | 'featured' | 'corporate'
                        )
                      }
              ...
              {/* Submit */}
              <button
                onClick={postIfListing}
                disabled={
                  !ifForm.title.trim() ||
                  !ifForm.description.trim() ||
                  ifPosting
                }
                className="w-full rounded-xl py-4 text-base font-bold transition-all disabled:cursor-not-allowed disabled:opacity-40"
                style={{
                ...
                {ifApplicants.map((a) => (
                  ...
                  >
                    ...
                    {a.status === 'applied' && (
                      <div className="flex gap-2">
                        <button
                          onClick={() => updateIfAppStatus(a.id, 'shortlisted')}
                        ...
                        <button
                          onClick={() => updateIfAppStatus(a.id, 'selected')}
                          className="flex-1 rounded-xl py-2 text-xs font-bold"
                          style={{
                        ...
                        <button
                          onClick={() => updateIfAppStatus(a.id, 'rejected')}
                          className="flex-1 rounded-xl py-2 text-xs font-bold"
                          style={{
