...
export function ManualCronButton({ jobId }: Props) {
  ...
  async function trigger() {
    ...
    try {
      ...
      if (!fnName) { setState('error'); return; }

      const body = jobId === 'admin-weekly-digest'
      ...
      await sb.from('cron_job_log').upsert(
        { job_id: jobId, last_run_at: new Date().toISOString(), status: res.ok ? 'ok' : 'error' },
        { onConflict: 'job_id' }
      );
  ...
  if (state === 'done')    return <span className="text-xs text-emerald-400 font-semibold">✓ Triggered</span>;
  if (state === 'error')   return <span className="text-xs text-red-400 font-semibold">✗ Failed</span>;
  if (state === 'running') return <span className="text-xs text-blue-400 animate-pulse">Running…</span>;
