...
export default async function SuspensionsPage() {
  ...
  const { data: suspended } = await admin
    .from('profiles')
    .select('id, full_name, email, suspension_status, suspension_tier, suspended_until, suspension_reason, last_suspended_at, is_banned')
  ...
  const tier2Count  = active.filter((u) => (u.suspension_tier as number) === 2).length;
  const tier3Count  = active.filter((u) => (u.suspension_tier as number) === 3).length;
  const tier4Count  = (suspended ?? []).filter((u) => u.is_banned as boolean).length;
  ...
  return (
    <div className="p-6 max-w-6xl space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <StatCard label="Appeals pending"    value={appealsCount ?? 0} dot={appealsCount ? 'amber' : undefined} />
      </div>

      {/* Active suspensions table */}
      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-800">
          <h2 className="text-sm font-semibold text-white">Active Suspensions</h2>
        </div>
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3.5">
                      <div className="text-sm text-white">{(u.full_name as string) ?? '—'}</div>
                      <div className="text-xs text-slate-500">{u.email as string}</div>
                    </td>
                    <td className="px-4 py-3.5">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${TIER_COLORS[tier] ?? TIER_COLORS[1]}`}>
                        {isBanned ? 'BANNED' : (TIER_LABELS[tier] ?? `T${tier}`)}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-xs text-slate-500">
                      {u.last_suspended_at
                        ? new Date(u.last_suspended_at as string).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })
                        : '—'}
                    </td>
                    <td className="px-4 py-3.5 text-xs">
                      {isBanned ? (
                        ...
                        <span className={expired ? 'text-slate-500 line-through' : 'text-slate-300'}>
                          {expires.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit' })}
                        </span>
                      ) : '—'}
                    </td>
                    <td className="px-4 py-3.5">
                      <div className="flex gap-3 justify-end">
                        {!isBanned && <LiftButton userId={u.id as string} name={(u.full_name as string) ?? 'User'} />}
                        {!isBanned && tier < 3 && <EscalateButton userId={u.id as string} name={(u.full_name as string) ?? 'User'} />}
                        {!isBanned && <BanButton userId={u.id as string} name={(u.full_name as string) ?? 'User'} />}
                      </div>
                    </td>
                  </tr>
                );
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        {/* Threshold Settings */}
        <div className="bg-slate-900 rounded-2xl border border-slate-800 p-5">
          <h2 className="text-sm font-semibold text-white mb-4">Violation Thresholds</h2>
          <form
