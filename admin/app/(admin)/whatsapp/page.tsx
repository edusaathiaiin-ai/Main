...
export default async function WhatsAppPage() {
  ...
  const { data: sessions } = await admin
    ...
    .select(
      'wa_phone, user_id, message_count_today, last_message_at, last_reset_date'
    )
  ...
  const activeNumbers = (sessions ?? []).filter((s) => {
    const last = s.last_message_at
      ? new Date(s.last_message_at as string)
      : null
    ...
    <div className="p-6 max-w-6xl space-y-8">
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 p-5">
        ...
        <div className="text-xs text-amber-400/70 bg-amber-500/5 border border-amber-500/15 rounded-xl px-4 py-2">
          Use broadcasts sparingly. Excessive messaging may cause users to block
          the number.
        </div>
      </div>
    </div>
