...
export default async function FinancialsPage() {
  ...
  const { data: sessionsThisMonth } = await admin
    ...
    .select(
      'fee_paise, platform_fee_paise, faculty_payout_paise, payout_status, status'
    )
    .eq('status', 'completed')
    .gte(
      'created_at',
  ...
  const sessionGmv = (sessionsThisMonth ?? []).reduce(
    (s, r) => s + ((r.fee_paise as number) ?? 0),
    0
  )
  const sessionPlatform = (sessionsThisMonth ?? []).reduce(
    (s, r) => s + ((r.platform_fee_paise as number) ?? 0),
    0
  )
  ...
  const liveGmv = (liveRevenue ?? []).reduce(
    (s, r) =>
      s +
      ((r.price_per_seat_paise as number) ?? 0) *
        ((r.seats_booked as number) ?? 0),
    0
  )
  ...
  for (const s of recentSubs ?? []) {
    ...
    dailyRevenue.set(
      date,
      (dailyRevenue.get(date) ?? 0) + ((s.amount_inr as number) ?? 0)
    )
  }
    ...
    <div className="p-6 max-w-6xl space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            ...
            value={fmtInrFromRupees(totalStudentMrr)}
            sub="Monthly + annual/12"
            accent
          />
          <StatCard
            label="Institution plans"
            value={fmtInrFromRupees(institutionMrr)}
          />
          <StatCard
            label="Marketplace (this mo)"
            value={fmtInr(sessionPlatform + livePlatform)}
            sub="Sessions + live 20% cut"
          />
          <StatCard
            label="Total MRR"
              ...
              {Array.from(payoutByFaculty.entries()).map(([fid, data]) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5 text-xs text-slate-500">
                    {data.lastPayout
                      ? new Date(data.lastPayout).toLocaleDateString('en-IN', {
                          day: '2-digit',
                          month: 'short',
                          year: '2-digit',
                        })
              ...
              {(refunds ?? []).map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <Link
                      href={`/users/${r.user_id as string}`}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {(facultyEarnings ?? []).map((fe) => {
                ...
                const tdsPaise = tdsApplies
                  ? Math.round(grossPaise * TDS_RATE)
                  : 0
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-xs text-red-400">
                      {tdsApplies ? fmtInr(tdsPaise) : '—'}
                    </td>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 p-5">
        ...
        <p className="text-xs text-slate-600 mt-3">
          Exports use the Razorpay dashboard for verified payment data. These
          links generate reports from our DB records.
        </p>
      </div>
    </div>
  )
