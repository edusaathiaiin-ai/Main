...
function MetricCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-5 ${accent ? 'bg-amber-500/10 border-amber-500/30' : 'bg-slate-900 border-slate-800'}`}
    >
...
export default async function RevenuePage() {
  ...
  const { data: allPayments } = await admin
    .from('subscriptions')
    .select(
      'id, razorpay_order_id, razorpay_payment_id, plan_id, amount_inr, status, created_at, user_id'
    )
            ...
            <tbody>
              {(allPayments ?? []).map((p: Record<string, unknown>) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-semibold px-2.5 py-1 rounded-full ${statusChip[p.status as string] ?? statusChip.created}`}
                    >
                      {p.status as string}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <Link
                      href={`/users/${p.user_id as string}`}
