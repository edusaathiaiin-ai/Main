...
export async function refundSubscription(
  ...
  const reason =
    (formData.get('reason') as string | null) ?? 'Admin-initiated refund'
  ...
  const { data: subRow, error: fetchErr } = await admin
    .from('subscriptions')
    .select(
      'id, user_id, plan_id, amount_inr, razorpay_payment_id, status, razorpay_refund_id'
    )
  ...
  if (sub.razorpay_refund_id)
    return { ok: false, error: 'Refund already issued' }

  const paymentId = sub.razorpay_payment_id
  if (!paymentId)
    return { ok: false, error: 'No razorpay_payment_id on this subscription' }
  ...
  if (!rzpRes.ok) {
    ...
    return {
      ok: false,
      error: `Razorpay error ${rzpRes.status}: ${errBody.slice(0, 200)}`,
    }
  }
