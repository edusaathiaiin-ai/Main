...
async function razorpayRefund(paymentId: string, amountPaise: number): Promise<string | null> {
  ...
  if (!res.ok) {
    console.error('sessions/actions: Razorpay refund API error', res.status, await res.text());
    return null;
  }
...
export async function releaseToFaculty(formData: FormData) {
  ...
  if (session.payout_status === 'released') {
    console.warn(`releaseToFaculty: session ${sessionId} already released — skipping`);
    return;
  }
  ...
  if (rpcErr) {
    console.error('releaseToFaculty: increment_faculty_earnings RPC failed:', rpcErr.message);
  }
...
export async function refundStudent(formData: FormData) {
  ...
  if (paymentId && refundPaise) {
    ...
    if (!razorpayRefundId) {
      console.error(`sessions/actions: Razorpay refund failed for session ${sessionId}`);
    }
  }
