...
export default async function SessionsPage() {
  ...
  const totalGmv    = all.reduce((sum, s) => sum + ((s.fee_paise as number) ?? 0), 0);
  const platformRev = all.reduce((sum, s) => sum + ((s.platform_fee_paise as number) ?? 0), 0);

  return (
    <div className="p-6 max-w-6xl space-y-8">
      <h1 className="text-xl font-bold text-white">Sessions — 1:1 Faculty Finder</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        ...
        <StatCard label="Disputed"        value={disputed.length} dot={disputed.length > 0 ? 'red' : undefined} accent={disputed.length > 0} />
        <StatCard label="Total GMV"       value={`₹${(totalGmv / 100).toLocaleString('en-IN')}`} />
        <StatCard label="Platform revenue" value={`₹${(platformRev / 100).toLocaleString('en-IN')}`} />
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {disputed.map((s) => {
                  ...
                  return (
                    <tr key={s.id as string} className="border-b border-red-500/10 hover:bg-red-900/10">
                      <td className="px-5 py-3.5 font-mono text-xs text-slate-500">
                        {(s.id as string).slice(0, 8)}
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="text-sm text-white">{(student?.full_name as string) ?? '—'}</div>
                        <div className="text-xs text-slate-500">{student?.email as string}</div>
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="text-sm text-white">{(faculty?.full_name as string) ?? '—'}</div>
                        <div className="text-xs text-slate-500">{faculty?.email as string}</div>
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-white font-semibold text-sm">
                        ₹{(((s.fee_paise as number) ?? 0) / 100).toLocaleString('en-IN')}
                      </td>
                ...
                return (
                  <tr key={s.id as string} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${STATUS_CHIP[status] ?? STATUS_CHIP.requested}`}>
                        {status}
                      </span>
                    </td>
                    ...
                    <td className={`px-4 py-3 text-xs font-medium ${PAYOUT_CHIP[payout] ?? 'text-slate-500'}`}>
                      {payout ?? '—'}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
