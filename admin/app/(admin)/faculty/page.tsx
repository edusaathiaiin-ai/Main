...
export default async function FacultyPage({
  ...
  const pending = (allFaculty ?? []).filter(
    (f) =>
      (f.verification_status as string) === 'pending' &&
      !(f.is_emeritus as boolean) &&
      (f.employment_status as string) !== 'independent'
  )
  const all = (allFaculty ?? []).filter(
    (f) =>
      !(f.is_emeritus as boolean) &&
      (f.employment_status as string) !== 'independent'
  )
  const emeritus = (allFaculty ?? []).filter((f) => f.is_emeritus as boolean)
  const independent = (allFaculty ?? []).filter(
    (f) =>
      (f.employment_status as string) === 'independent' &&
      (f.verification_status as string) === 'pending'
  )
  ...
  const { data: pendingPayouts } = await admin
    ...
    .select(
      'faculty_id, faculty_payout_paise, fee_paise, status, payout_status, completed_at'
    )
  ...
  for (const s of pendingPayouts ?? []) {
    ...
    payoutByFaculty.set(
      fid,
      (payoutByFaculty.get(fid) ?? 0) + (s.faculty_payout_paise as number)
    )
  }
  ...
  const { data: facultyExtras } = await admin
    ...
    .in(
      'user_id',
      Array.from(payoutByFaculty.keys()).length
        ? Array.from(payoutByFaculty.keys())
        : ['__none__']
    )
    ...
    <div className="p-6 max-w-6xl space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <StatCard
          ...
          dot={independent.length > 0 ? 'amber' : undefined}
        />
        <StatCard
          label="Payouts pending"
          value={payoutByFaculty.size}
          dot={payoutByFaculty.size > 0 ? 'amber' : undefined}
        />
      </div>
                  ...
                  <div className="space-y-1 flex-1">
                    ...
                    {(f.institution_name as string) ? (
                      <p className="text-xs mt-1 text-emerald-400">
                        ✅ Known domain — {f.institution_name as string}
                      </p>
                    ) : (profile?.email as string)?.endsWith('.ac.in') ||
                      (profile?.email as string)?.includes('.edu') ? (
                      <p className="text-xs mt-1 text-amber-400">
                        🟡 Academic email — manual check advised
                      </p>
                    ) : (
                      <p className="text-xs mt-1 text-slate-500">
                        ⚠️ Unknown domain — verify independently
                      </p>
                    )}
                  </div>
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {TAB_ITEMS.map((f) => {
                  const profile = f.profiles as unknown as Record<
                    string,
                    unknown
                  >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-xs text-slate-400">
                        {(profile?.primary_saathi_id as string) ?? '—'}
                      </td>
                      <td className="px-4 py-3.5">
                        <span
                          className={`text-xs font-semibold px-2 py-0.5 rounded-full ${STATUS_CHIP[status] ?? STATUS_CHIP.pending}`}
                        >
                          {status}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-xs text-slate-400 capitalize">
                        {((profile?.plan_id as string) ?? 'free').replace(
                          /-/g,
                          ' '
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-xs text-slate-300">
                        ₹
                        {(
                          ((f.total_earned_paise as number) ?? 0) / 100
                    ...
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          <span className="text-amber-400 text-xs font-bold px-2 py-0.5 bg-amber-500/10 rounded-full">
                            Emeritus
                          </span>
                      ...
                      <td className="px-4 py-3.5 text-xs text-slate-400">
                        {(f.former_institution as string) ??
                          (f.institution as string) ??
                          '—'}
                        {f.retirement_year ? (
                          <span className="block text-slate-600">
                            Retired {f.retirement_year as number}
                          </span>
                        ) : null}
                      </td>
                      ...
                      <td className="px-4 py-3.5 flex gap-2">
                        {docUrl && (
                          <DownloadDocButton userId={f.user_id as string} />
                        )}
                        {status === 'pending' && (
                          <VerifyButton
                            userId={f.user_id as string}
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-teal-400 text-xs font-bold px-2 py-0.5 bg-teal-500/10 rounded-full">
                        Independent
                      </span>
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
            ...
            <div className="text-xs text-slate-400">
              Total: ₹
              {(
                Array.from(payoutByFaculty.values()).reduce(
                  (a, b) => a + b,
                  0
                ) / 100
              ).toLocaleString('en-IN')}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(facultyExtras ?? []).map((fe) => {
                  const profile = fe.profiles as unknown as Record<
                    string,
                    unknown
                  >
