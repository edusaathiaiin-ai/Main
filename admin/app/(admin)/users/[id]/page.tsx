...
const FOUNDING_PERIOD_END = new Date(
  process.env.FOUNDING_PERIOD_END ?? '2026-09-01'
)
...
export default async function SubscriberDetailPage({
  ...
  const rzpBaseUrl =
    process.env.NEXT_PUBLIC_RAZORPAY_DASHBOARD_URL ??
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
        ...
        <Section title="Subscription">
          <Row label="Plan">
            <span className="text-white capitalize">
              {profile.plan_id?.replace(/-/g, ' ') ?? 'free'}
            </span>
          </Row>
          <Row label="Status">
            <span
              className={`text-xs font-semibold px-2.5 py-1 rounded-full ${STATUS_BADGE[profile.subscription_status] ?? STATUS_BADGE.inactive}`}
            >
          ...
          {profile.subscription_expires_at && (
            <>
              ...
              <Row label="Days remaining">
                <span
                  className={`text-white font-semibold ${days === 0 ? 'text-red-400' : ''}`}
                >
                  {days === 0
                    ? 'Expired'
                    : days !== null
        ...
        {/* ── Usage ────────────────────────────────────────────────── */}
        <Section title="Usage">
          <Row label="Total chat sessions">
            <span className="text-white">{totalSessions ?? 0}</span>
          </Row>
          <Row label="Saathis joined">
            <span className="text-white">{saathisJoined ?? 0}</span>
          </Row>
          <Row label="Check-ins completed">
            <span className="text-white">{checkinsCount ?? 0}</span>
          </Row>
          <Row label="Last active">
            <span className="text-white">
              {lastActive ? fmtDate(lastActive) : 'Never'}
            </span>
          </Row>
        </Section>
      </div>
              ...
              <tbody>
                {payments.map((p: Record<string, unknown>) => (
                  ...
                  >
                    ...
                    <td className="py-3 pr-6">
                      <CopyField
                        value={p.razorpay_subscription_id as string | null}
                      />
                    </td>
                    <td className="py-3 pr-4 text-white font-medium">
                      {p.amount_inr
                        ? `₹${(p.amount_inr as number).toLocaleString('en-IN')}`
                        : '—'}
                    </td>
                    ...
                    <td className="py-3">
                      <span
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full ${STATUS_BADGE[p.status as string] ?? STATUS_BADGE.inactive}`}
                      >
                        {p.status as string}
                      </span>
                    </td>
